...
export default function SeasonPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <div className="mb-8 flex items-end justify-between flex-wrap gap-4">
        ...
        {registered && !season && (
          <div className="flex items-center gap-3">
            ...
            />
            <button onClick={handleCreateSeason} disabled={creating} className="btn-primary text-sm">
                  ...
                  <Link href={`/track/${t}`}
                        className={`flex items-center gap-2.5 rounded-lg px-4 py-2.5 flex-shrink-0
                                   ...
                                   ${isCompleted ? "bg-green-500/10" :
                                     isNext ? "bg-f1-red/10 ring-1 ring-f1-red/30" :
                                     "bg-pit-surface"}`}>
                    <span className={`w-6 h-6 rounded-full flex items-center justify-center
                                     text-[10px] font-extrabold ${
                      isCompleted ? "bg-green-500/20 text-green-400" :
                      isNext ? "bg-f1-red/20 text-f1-red" :
                    ...
                    <span className={`text-sm font-semibold whitespace-nowrap ${
                      ...
                      isNext ? "text-white" : "text-pit-text"
                    }`}>
      ...
      {tab === "championship" ? (
        <ChampionshipTable standings={standings} tracks={tracks} completedTracks={completedTracks} />
      ) : (
...
function ChampionshipTable({
  ...
  return (
    <div className="card overflow-hidden">
      ...
      <div>
        {standings.map((s, idx) => (
          <Link key={s.player_id} href={`/player/${s.username}`}
            ...
            <span className="w-14 text-right font-extrabold text-white tabular-nums">
              {s.total_points}
            </span>
            <span className="w-10 text-right text-pit-text tabular-nums font-mono text-xs">
              {s.wins}
            </span>
            <span className="w-10 text-right text-pit-text tabular-nums font-mono text-xs">
              {s.podiums}
            </span>
            <span className="w-10 text-right text-pit-muted tabular-nums font-mono text-xs">
              {s.races}
            </span>
            <span className="w-14 text-right text-pit-muted tabular-nums font-mono text-xs">
              {Math.round(s.elo)}
            </span>
          </Link>
        ))}
