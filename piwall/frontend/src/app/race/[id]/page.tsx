    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-10">
        ...
        <div className="flex gap-3">
          <button onClick={handleJoin} disabled={joining} className="btn-secondary" type="button">
            {joining ? "Joining..." : "Join Race"}
          </button>
          <button onClick={handleStart} disabled={starting || players.length === 0} className="btn-primary" type="button">
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <div className="text-7xl font-black text-f1-red tabular-nums">{countdown}</div>
          <div className="text-pit-muted text-xs uppercase tracking-[0.3em] mt-2">Lights Out In...</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center animate-fade-in">
          <div className="flex items-center justify-center gap-3 mb-6">
            {[5, 4, 3, 2, 1].map((n) => (
              <div key={n} className="w-10 h-10 rounded-full border-2 bg-transparent border-pit-border" />
            ))}
          </div>
          <div className="text-3xl sm:text-5xl font-black text-green-500 tracking-tight">LIGHTS OUT</div>
          <div className="text-lg sm:text-xl font-bold text-pit-light mt-1">AND AWAY WE GO!</div>
          <div className="mt-4">
    ...
    <div className="max-w-7xl mx-auto px-4 py-4">
      <div className="card px-4 sm:px-5 py-3 mb-4 flex items-center gap-3 sm:gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          <div className="w-1 h-6 rounded-full bg-f1-red" />
          <span className="text-sm font-extrabold text-white tracking-wide">{raceInfo?.track?.toUpperCase() || "RACE"}</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        <div className="lg:col-span-3">
          {cars.length > 0 && <Leaderboard cars={cars} previousPositions={getPrevPositions()} />}
        </div>
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          {cars.length > 0 && <BeliefPanel cars={cars} />}
        </div>
      </div>
      ...
      {status === "finished" && result && (
        <div className="space-y-4 mt-6">
            ...
            <div className="p-5 space-y-1">
              {[...result.standings]
                ...
                .map((car, idx) => (
                  ...
                  >
                    <span
                      className={`w-10 text-right font-extrabold tabular-nums ${
          ...
          <div className="card p-5 flex items-center justify-between gap-4 flex-wrap">
            <div>
              <div className="section-label mb-1">Account History</div>
              <p className="text-sm text-pit-text">
                Log this result from the lobby or review your saved submissions and race records on the account page.
              </p>
            </div>
